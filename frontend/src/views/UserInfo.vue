...
const userInfo = reactive({
  username: '',
  phone: '',
  avatar: '@/assets/default.png'  // 默认头像
})
...
const fetchUserInfo = async () => {
  try {
    ...
    userInfo.avatar = avatarRes.avatar
  } catch (error) {
    console.error('获取用户信息失败:', error)
    ElMessage.error('获取用户信息失败')
  }
...
const handleAvatarUpload = async (file) => {
  try {
    ...
    formData.append('file', file.file); // 注意：参数名必须和后端一致（文档6中是`file`）

    // 调用文档6中的接口
    const uploadRes = await api.post('/upload-avatar', formData, {
      headers: {
        'Content-Type': 'multipart/form-data' // 必须设置multipart格式
      }
    })

    // 更新头像URL（假设接口返回格式为 { url: "xxx" }）
